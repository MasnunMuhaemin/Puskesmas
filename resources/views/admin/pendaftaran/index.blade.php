...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Kelola antrian dan pendaftaran pasien hari ini.</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
        <form action="{{ route(Auth::user()->role . '.pendaftaran.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <select name="status" class="w-full bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-primary">
            ...
            <div>
                ...
                <input type="date" name="tanggal" value="{{ request('tanggal', date('Y-m-d')) }}" class="w-full bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-primary">
            </div>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/40 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <form action="{{ route('pendaftaran.update-status', $item->id) }}" method="POST" class="inline">
                                @csrf
                                <select name="status" onchange="this.form.submit()" class="text-xs bg-gray-50 border-none rounded-lg focus:ring-primary py-1">
